import evidence from "@/fixtures/v16-tools-care-sanitized-20260920.json";
import { z } from "zod";
...
const rowSchema = z.object({
  ...
  category: z.enum(["herramientas","cuidado-personal-salud"]),
  subcategory: z.enum(["taladros","amoladoras","sierras"]).nullable(),
...
const products = Object.freeze(parsed.products.map((row): Product => Object.freeze({
  id: `v16-tools-care:${row.id}`,
  slug: `${slugPart(row.name)}-tools-care-${slugPart(row.id)}`,
  name: row.name,
  brand: row.brand,
  model: row.model,
  ...
  warranty: null,
  visible: true,
  source: "v16-tools-care-sanitized-snapshot",
})));

export const v16ToolsCareSnapshotEvidence = Object.freeze({
...
export class V16ToolsCareSnapshotCatalogAdapter implements CatalogAdapter {
  readonly source = "v16-tools-care-sanitized-snapshot" as const;
...
export const v16ToolsCareSnapshotProducts = products;
