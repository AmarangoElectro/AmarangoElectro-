import evidence from "@/fixtures/v16-catalog-expansion-31-literal-brand-20260921.json";
import { z } from "zod";
...
const rowSchema = z.object({
  ...
  brand_evidence: z.literal("literal_known_brand_in_same_category"),
  image_evidence: z.string().min(1),
}).strict();

const evidenceSchema = z.object({
  evidence_status: z.literal("publication_gate_literal_brand_expansion"),
  ...
  parsedEvidence.products.map(
    (row): Product =>
      freezeProduct({
        ...
        warranty: null,
        visible: true,
        source: "v16-catalog-expansion-31-literal-brand",
      }),
  ),
);
...
export class V16CatalogExpansion31LiteralBrandAdapter implements CatalogAdapter {
  readonly source = "v16-catalog-expansion-31-literal-brand" as const;
