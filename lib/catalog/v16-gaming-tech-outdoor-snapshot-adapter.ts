import evidence from "@/fixtures/v16-gaming-tech-outdoor-sanitized-20260920.json";
import { z } from "zod";
...
const rowSchema = z.object({
  ...
  category: z.enum(["gaming","tecnologia-accesorios","camping-aire-libre-mascotas","audio"]),
  subcategory: z.enum(["playstation","cargadores-y-accesorios","camping-y-aire-libre","auriculares","parlantes-portatiles"]).nullable(),
...
const products = Object.freeze(parsed.products.map((row): Product => Object.freeze({
  id: `v16-gaming-tech-outdoor:${row.id}`,
  slug: `${slugPart(row.name)}-gto-${slugPart(row.id)}`,
  name: row.name,
  brand: row.brand,
  model: row.model,
  ...
  warranty: null,
  visible: true,
  source: "v16-gaming-tech-outdoor-snapshot",
})));

export const v16GamingTechOutdoorSnapshotEvidence = Object.freeze({
...
export class V16GamingTechOutdoorSnapshotCatalogAdapter implements CatalogAdapter {
  readonly source = "v16-gaming-tech-outdoor-snapshot" as const;
...
export const v16GamingTechOutdoorSnapshotProducts = products;
