import evidence from "@/fixtures/v16-catalog-expansion-50-global-brand-storage-20260921.json";
import { z } from "zod";
...
const rowSchema = z.object({
  ...
  brand_evidence: z.literal("literal_known_brand_any_category"),
  image_evidence: z.literal("usable_by_storage_metadata"),
}).strict();

const evidenceSchema = z.object({
  evidence_status: z.literal("publication_gate_global_literal_brand_storage_expansion"),
  ...
  product_count: z.literal(50),
  products: z.array(rowSchema).length(50),
}).strict();
...
const products = Object.freeze(
  parsedEvidence.products.map(
    (row): Product =>
      freezeProduct({
        id: `exp50:${row.id}`,
        slug: `${slugPart(row.name)}-exp50-${slugPart(row.id)}`,
        name: row.name,
        brand: row.brand,
        model: null,
        ...
        warranty: null,
        visible: true,
        source: "v16-catalog-expansion-50-global-brand-storage",
      }),
  ),
);

export const v16CatalogExpansion50Evidence = Object.freeze({
  status: parsedEvidence.evidence_status,
  sourceReference: parsedEvidence.source_reference,
  productCount: products.length,
});

export class V16CatalogExpansion50GlobalBrandStorageAdapter implements CatalogAdapter {
  readonly source = "v16-catalog-expansion-50-global-brand-storage" as const;
...
export const v16CatalogExpansion50Products = products;
