import evidence from "@/fixtures/v16-smart-tv-audio-sanitized-20260920.json";
import { z } from "zod";
...
const rowSchema = z.object({
  ...
  category: z.enum(["smart-tv","audio","electrodomesticos"]),
  subcategory: z.enum(["parlantes-portatiles","torres","barras-de-sonido","auriculares","home-audio","lavado"]).nullable(),
...
const products = Object.freeze(parsed.products.map((row): Product => Object.freeze({
  id: `v16-media:${row.id}`,
  slug: `${slugPart(row.name)}-media-${slugPart(row.id)}`,
  name: row.name,
  brand: row.brand,
  model: row.model,
  ...
  warranty: null,
  visible: true,
  source: "v16-media-sanitized-snapshot",
})));

export const v16MediaSnapshotEvidence = Object.freeze({
...
export class V16MediaSnapshotCatalogAdapter implements CatalogAdapter {
  readonly source = "v16-media-sanitized-snapshot" as const;
...
export const v16MediaSnapshotProducts = products;
