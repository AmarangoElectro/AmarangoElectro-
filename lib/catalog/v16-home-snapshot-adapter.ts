...
const rowSchema = z.object({
  ...
  category: z.enum(["hogar","gaming","tecnologia-accesorios","camping-aire-libre-mascotas"]),
  subcategory: z.enum(["hogar-y-deco","bazar-y-mesa","blanqueria","camping-y-aire-libre"]).nullable(),
...
const products = Object.freeze(parsed.products.map((row): Product => Object.freeze({
  id: `v16-home:${row.id}`,
  slug: `${slugPart(row.name)}-home-${slugPart(row.id)}`,
  name: row.name,
  brand: row.brand,
  model: row.model,
  ...
  warranty: null,
  visible: true,
  source: "v16-home-sanitized-snapshot",
})));

export const v16HomeSnapshotEvidence = Object.freeze({
...
export class V16HomeSnapshotCatalogAdapter implements CatalogAdapter {
  readonly source = "v16-home-sanitized-snapshot" as const;
...
export const v16HomeSnapshotProducts = products;
