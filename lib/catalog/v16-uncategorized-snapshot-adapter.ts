import evidence from "@/fixtures/v16-uncategorized-sanitized-20260920.json";
import { z } from "zod";
...
const rowSchema = z.object({
  ...
  category: z.enum(["hogar","electrodomesticos","otros"]),
  subcategory: z.enum(["hogar-y-deco","bazar-y-mesa","pequenos-electrodomesticos","coccion"]).nullable(),
  source_category: z.literal(""),
...
const products = Object.freeze(parsed.products.map((row): Product => Object.freeze({
  id: `v16-uncat:${row.id}`,
  slug: `${slugPart(row.name)}-uncat-${slugPart(row.id)}`,
  name: row.name,
  brand: row.brand,
  model: row.model,
  ...
  warranty: null,
  visible: true,
  source: "v16-uncategorized-snapshot",
})));

export const v16UncategorizedSnapshotEvidence = Object.freeze({
...
export class V16UncategorizedSnapshotCatalogAdapter implements CatalogAdapter {
  readonly source = "v16-uncategorized-snapshot" as const;
...
export const v16UncategorizedSnapshotProducts = products;
