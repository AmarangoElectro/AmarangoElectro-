...
const operationalPanels = [
  ...
  "components/internal/admin/providers-panel.tsx",
  "components/internal/admin/provider-inbox-panel.tsx",
];
...
test("Administration operational panels keep human-facing status copy", async () => {
  ...
  for (const oldCopy of [
    ...
    "Solo lectura · vía RPC segura (`v16_providers_list`)",
    "Solo lectura/escritura vía RPC segura · sin acceso directo a tablas legacy",
  ]) {
  ...
  for (const expected of [
    ...
    "Conexión segura · proveedores reales",
    "Conexión segura · seguimientos reales con proveedores",
  ]) {
...
test("Administration operational panels preserve safe adapter boundaries", async () => {
  ...
  const providers = await source("components/internal/admin/providers-panel.tsx");
  const inbox = await source("components/internal/admin/provider-inbox-panel.tsx");
  ...
  assert.match(providers, /createProviderInboxAdapter/);
  assert.match(inbox, /createProviderInboxAdapter/);
});
...
test("Administration mobile-depth CSS protects 320-412 layouts and dark surfaces", async () => {
  ...
  assert.match(css, /V16 Administration — Providers, Calculator and Plates mobile polish/);
  assert.match(css, /\.admin-calculator-modes/);
  assert.match(css, /\.admin-plate-copy/);
  assert.match(css, /\.providers-area/);
});
