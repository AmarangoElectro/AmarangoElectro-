...
# Windows-native npm.exe reports the cache path in native drive-letter form
# (e.g. C:\...) even when it was set via a Git-Bash POSIX path (/c/...). Use
# cygpath, when present, to normalize before comparing so this check still
# verifies "same directory" instead of failing on cosmetic path-style
# differences. Falls back to a plain string compare if cygpath is unavailable.
# Does not touch V4.18B feature scope.
if command -v cygpath >/dev/null 2>&1; then
  actual_cache="$(cygpath -u "${actual_cache}")"
fi
