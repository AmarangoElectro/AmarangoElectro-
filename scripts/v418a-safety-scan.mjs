...
const scannedFiles = ["lib/internal/admin/v418a-quick-actions.ts","components/internal/admin/v418a-quick-actions-sheet.tsx","components/internal/admin/admin-product-card.tsx","components/internal/admin/admin-product-grid.tsx"];
const source = (await Promise.all(scannedFiles.map((file) => readFile(file,"utf8")))).join("\n");
const patterns = {
  insert:/\.insert\s*\(/gi, update:/\.update\s*\(/gi, upsert:/\.upsert\s*\(/gi, delete:/\.delete\s*\(/gi,
  writeRpc:/\.rpc\s*\([^)]*(?:write|save|publish|sync|mutate)/gi, mutatingHttp:/\bmethod\s*:\s*["'](?:POST|PUT|PATCH|DELETE)["']/gi,
  remoteStorageWrite:/\.storage\.(?:upload|remove|move|copy|update)/gi, network:/\bfetch\s*\(/gi,
...
for (const [file,expected] of Object.entries(protectedFiles)) { const actual=createHash("sha256").update(await readFile(file)).digest("hex"); protectedHashes[file]={expected,actual,passed:actual===expected}; }
const branch=execFileSync("git",["branch","--show-current"],{encoding:"utf8"}).trim();
const hasMainBranch=execFileSync("git",["branch","--list","main"],{encoding:"utf8"}).trim().length>0;
const passed=branch.startsWith("recovery/amarango-v16-v418a-")&&!hasMainBranch&&Object.values(counts).every((count)=>count===0)&&Object.values(protectedHashes).every((entry)=>entry.passed);
process.stdout.write(`${JSON.stringify({gate:"FASE G · V4.18A",mode:"draft_preview_write_gate_blocked",branch,hasMainBranch,scannedFiles,counts,protectedHashes,productionOrDeployExecuted:false,v418bStarted:false,v419Started:false,foundationStarted:false,passed},null,2)}\n`);
