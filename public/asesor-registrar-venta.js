...
(function(){
  'use strict';
  var VERSION='asesor-venta-2026-08-23-1';
  ...
  function opcionesCuotas(){
    var arr=[];
    try{if(typeof window.cuotasActivas==='function')arr=window.cuotasActivas()||[];}catch(e){}
    arr=(Array.isArray(arr)?arr:[]).map(Number).filter(function(n){return n>0&&n<100;});
    if(!arr.length)arr=[3,6,9,12];
    var out='<option value="0">Contado</option>';
    arr.forEach(function(n){out+='<option value="'+n+'">'+n+' cuotas</option>';});
    return out;
  }
  ...
  function crearUI(){
    ...
    ov.innerHTML='<div class="ae-av-sheet">'
      ...
      +'<div class="ae-av-card"><div class="ae-av-sec">📦 Venta</div><div class="ae-av-grid">'
        ...
        +'<div class="ae-av-field"><label>Cuotas</label><select id="ae-av-cuotas">'+opcionesCuotas()+'</select></div>'
    ...
    ov.querySelector('.ae-av-x').onclick=cerrar;ov.addEventListener('click',function(e){if(e.target===ov)cerrar();});
  ...
  function limpiar(){['ae-av-cliente','ae-av-tel','ae-av-dni','ae-av-dir','ae-av-loc','ae-av-email','ae-av-prod','ae-av-precio','ae-av-senia'].forEach(function(id){var e=document.getElementById(id);if(e)e.value='';});var q=document.getElementById('ae-av-cuotas');if(q)q.value='0';actualizarTotal();}

  function clienteActual(){return {nombre:val('ae-av-cliente'),tel:val('ae-av-tel'),telefono:val('ae-av-tel'),dni:val('ae-av-dni'),direccion:val('ae-av-dir'),localidad:val('ae-av-loc'),email:val('ae-av-email')};}
  ...
  async function guardarEnHistorial(reg){
    // Preferimos el mecanismo nativo ya existente para mantener el mismo formato que Administración.
  ...
  async function guardarVenta(){
    if(!rolAsesor())return;
    var cli=clienteActual(),prod=val('ae-av-prod'),precio=num('ae-av-precio'),cuotas=Number(val('ae-av-cuotas')||0),senia=num('ae-av-senia');
    if(!cli.nombre){toast('Falta el nombre del cliente.');document.getElementById('ae-av-cliente')&&document.getElementById('ae-av-cliente').focus();return;}
    if(!prod){toast('Falta indicar el producto.');document.getElementById('ae-av-prod')&&document.getElementById('ae-av-prod').focus();return;}
    if(!(precio>0)){toast('Ingresá el precio de venta.');document.getElementById('ae-av-precio')&&document.getElementById('ae-av-precio').focus();return;}
    var a=asesorActual(),cuotaValor=0;
    if(cuotas>0){try{cuotaValor=typeof window.valorCuota==='function'?Number(window.valorCuota(precio,cuotas)||0):precio/cuotas;}catch(e){cuotaValor=precio/cuotas;}}
    var meta={origen:'vendedor',vendedor:a.nombre,vendedorId:a.id,precioVenta:precio,cuotas:cuotas,cuotaValor:cuotaValor,senia:senia,saldo:Math.max(0,precio-senia)};
    var reg={id:'V'+Date.now()+'_'+Math.random().toString(36).slice(2,6),fecha:new Date().toISOString(),tipo:'venta',cliente:cli.nombre,tel:cli.tel||'',localidad:cli.localidad||'',direccion:cli.direccion||'',dni:cli.dni||'',email:cli.email||'',envio:'',medioPago:'',destinatario:'Vendedor',total:precio,items:[{nombre:prod,precio:precio,qty:1}],origen:'vendedor',vendedor:a.nombre,vendedorId:a.id,precioVenta:precio,cuotas:cuotas,cuotaValor:cuotaValor,senia:senia,saldo:Math.max(0,precio-senia),_cliente:cli,_meta:meta};
  ...
  function insertarEnMenu(){
    ...
    var item=document.createElement('div');item.id='ae-av-menu-item';item.className=(cierre.classList&&cierre.classList.contains('mas-row'))?'mas-row ae-av-menurow':'ae-av-menurow';item.innerHTML='<span style="font-size:1.25rem;width:28px;text-align:center">🧾</span><span style="font-weight:900">Registrar venta</span>';item.onclick=function(e){e.preventDefault();e.stopPropagation();abrir();};cierre.parentElement.insertBefore(item,cierre);
  }
