...
(function(){
  'use strict';
  var VERSION='cliente-ui-comercial-2026-08-24-2';
  ...
  function botonMenuVisible(){
    var sels=['.hdr-menu-btn','[aria-label*="menú" i]','[aria-label*="menu" i]','button[onclick*="abrirMas"]','button[onclick*="menu" i]'];
    for(var i=0;i<sels.length;i++){try{var x=document.querySelector(sels[i]);if(x&&x.offsetParent!==null)return x;}catch(e){}}
    var bs=Array.from(document.querySelectorAll('button')).filter(function(b){return b.offsetParent!==null&&/^(☰|≡|☷)$/.test(String(b.textContent||'').trim());});
    return bs[0]||null;
  }
  function posicionarShare(b){
    if(!b)return;
    var m=botonMenuVisible();
    if(m&&m.getBoundingClientRect){
      var r=m.getBoundingClientRect();
      if(r.width>20&&r.top>=55&&r.top<260){
        b.style.top=Math.round(r.top)+'px';b.style.left=Math.max(8,Math.round(r.left-46))+'px';b.style.right='auto';return;
      }
    }
    b.style.top='76px';b.style.right='14px';b.style.left='auto';
  }
  ...
  function instalarShare(){
    ...
    if(!propio){
      propio=document.createElement('button');propio.id='ae-share-store-public-btn';propio.type='button';propio.setAttribute('aria-label','Compartir tienda AmarangoElectro');propio.setAttribute('title','Compartir tienda');propio.innerHTML=ICON;
      ...
      var svg=propio.querySelector('svg');if(svg)svg.style.cssText='width:20px;height:20px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round;stroke-linejoin:round';
      propio.onclick=function(ev){ev.preventDefault();ev.stopPropagation();try{if(navigator.vibrate)navigator.vibrate(6);}catch(e){}compartirTienda();};
  ...
  function instalar(){
    sync();
    if(!obs&&document.body){obs=new MutationObserver(sync);obs.observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['class','style']});}
