...
(function(){
  ...
  function capturarReferido(){
    var ref='';try{ref=codigo(new URL(window.location.href).searchParams.get('ref')||'');}catch(e){}
    if(!ref||ref===codigoActual())return;
  ...
  function referidoEntrante(){
    ...
    var r=codigo(ls(K_REF));return r&&r!==codigoActual()?r:'';
  }

  function guardarClientePublico(c){
    if(!c||!tel(c.telefono))return;
    setTimeout(function(){var previo=identidad();if(tel(previo.telefono)!==tel(c.telefono))return;setJson(K_IDENTIDAD,Object.assign({},previo,c,{actualizado:Date.now()}));},0);
  }
      ...
      try{
        ...
        if(metodo==='POST'&&/\/api\/cliente-identidad(?:\?|$)/.test(url)&&init&&typeof init.body==='string'){
          ...
          if(String(body&&body.accion||'').toLowerCase()==='registrar'){
            var ref=referidoEntrante();if(ref){body.referidoPorCodigo=ref;body.cliente=Object.assign({},body.cliente||{},{referidoPorCodigo:ref});}
            init=Object.assign({},init,{body:JSON.stringify(body)});tocar=true;
          }else tocar=true;
        }
      }catch(e){}
      var r=await orig(input,init);
      if(tocar){try{var d=await r.clone().json();if(d&&d.ok&&d.cliente)guardarClientePublico(d.cliente);}catch(e){}}
      return r;
    };
  ...
  async function hidratar(){
    var id=identidad();if(codigo(id.codigoReferido))return id;if(!tel(id.telefono))return id;if(HIDRATANDO)return HIDRATANDO;
  ...
  capturarReferido();instalarFetch();setTimeout(hidratar,900);
  window.AmarangoReferidos={referidoEntrante:referidoEntrante,codigoActual:codigoActual,hidratar:hidratar,decorarUrl:decorarUrlSync,obtenerUrl:obtenerUrl};
})();
