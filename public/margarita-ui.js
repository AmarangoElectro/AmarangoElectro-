...
const MARGARITA_SIDEBAR_VERSION = "sidebar-2026-08-23-6";
...
function instalarMargaritaSidebar() {
  ...
  if (!document.getElementById("margarita-sidebar-estilos")) {
    ...
    style.textContent = `
      ...
      .mg-bot-avatar{
        width:36px!important;height:36px!important;flex:0 0 36px!important;border-radius:50%!important;object-fit:cover!important;
      ...
      #mg-admin-modal{
        position:fixed;inset:0;z-index:10020;background:rgba(2,8,20,.58);backdrop-filter:blur(7px);-webkit-backdrop-filter:blur(7px);
        display:none;align-items:center;justify-content:center;padding:18px;
      }
      #mg-admin-modal.on{display:flex}
      .mg-admin-card{
        width:min(390px,94vw);background:linear-gradient(180deg,#fff 0%,#f8fbff 100%);border:1px solid rgba(11,45,107,.10);
        border-radius:24px;padding:20px;box-shadow:0 24px 70px rgba(0,0,0,.34);font-family:inherit;
      }
      ...
      .mg-admin-cerrar{margin-top:17px;width:100%;border:0;border-radius:13px;background:linear-gradient(135deg,#0B2D6B,#1e5bc8);color:#fff;padding:12px;font-weight:900;cursor:pointer;font-family:inherit}
      .mg-admin-ok{font-size:.67rem;color:#059669;font-weight:800;margin-top:10px;min-height:16px;text-align:center}

      /* La portada y “Ver todo” respetan la misma vista de tarjetas. */
      .portada-cat-grid.vista-lista{grid-template-columns:1fr!important}
      .portada-cat-grid.vista-lista .prod-foto{height:240px!important}
      .portada-cat-grid.vista-chica{grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:7px!important}
      ...
      .portada-cat-grid.vista-chica .prod-nombre{font-size:.64rem!important;min-height:auto!important}
      .portada-cat-grid.vista-chica .calc-btn{font-size:.58rem!important;padding:6px 2px!important}
  ...
  function crearSelectorRapido() {
    ...
    switcher.querySelectorAll("[data-mg-formato]").forEach((b) => {
      b.addEventListener("click", (ev) => {
        ev.preventDefault();ev.stopPropagation();guardarFormato(b.dataset.mgFormato);
      });
    });
    return switcher;
  }
  ...
  function asegurarControlesAdmin() {
    ...
    if (admin && !btn) {
      ...
      btn.type = "button";btn.className = "mg-admin-config-btn";btn.title = "Configurar Margarita";
      btn.setAttribute("aria-label", "Configurar Margarita");btn.textContent = "⚙️";
  ...
  function decorarBurbujaBot(burbuja) {
    ...
    const fila = document.createElement("div");fila.className = "mg-bot-row";
    const avatar = document.createElement("img");avatar.className = "mg-bot-avatar";avatar.src = MARGARITA_AVATAR;avatar.alt = "Margarita";
    burbuja.parentNode.insertBefore(fila, burbuja);fila.appendChild(avatar);fila.appendChild(burbuja);
  }
  ...
  function ajustarViewportRapido() {
    ajustarViewport();requestAnimationFrame(ajustarViewport);setTimeout(ajustarViewport, 18);
  }

  window.margaritaAbrir = function () {
    clearTimeout(cierreTimer);overlay.style.display = "block";scrollBase = window.scrollY || 0;
    ...
    requestAnimationFrame(() => {
      overlay.classList.add("abierto");decorarMensajes();asegurarControlesAdmin();ajustarViewportRapido();mensajes.scrollTop = mensajes.scrollHeight;
    });
  };

  window.margaritaCerrar = function () {
    overlay.classList.remove("abierto");clearTimeout(cierreTimer);
    cierreTimer = setTimeout(() => {
      if (!abierto()) {
        overlay.style.display = "none";panel.style.removeProperty("top");panel.style.removeProperty("bottom");panel.style.removeProperty("height");
      }
    }, 300);
  };
  ...
  input.addEventListener("focus", () => { scrollBase = window.scrollY || 0;ajustarViewportRapido();mensajes.scrollTop = mensajes.scrollHeight; });
  input.addEventListener("blur", () => requestAnimationFrame(ajustarViewport));
  if (vv) { vv.addEventListener("resize", ajustarViewportRapido);vv.addEventListener("scroll", ajustarViewportRapido); }
  ...
  setInterval(() => {
    aplicarPreferencias();asegurarControlesAdmin();sincronizarVistaTarjetas();
  }, 900);
