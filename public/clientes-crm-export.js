...
(function(){
  'use strict';
  var VERSION='clientes-crm-export-2026-08-23-1';
  ...
  function lineaCRM(c){
    var out=[];out.push('Nombre: '+s(c.nombre));if(c.apodo)out.push('Nombre preferido: '+s(c.apodo));if(c.telefono||c.tel)out.push('Teléfono: '+s(c.telefono||c.tel));if(c.dni)out.push('DNI: '+s(c.dni));if(c.mail||c.email)out.push('Email: '+s(c.mail||c.email));if(c.domicilio||c.dir)out.push('Dirección: '+s(c.domicilio||c.dir));if(c.localidad||c.loc)out.push('Localidad: '+s(c.localidad||c.loc));if(c.redTipo||c.redUser)out.push('Red: '+[s(c.redTipo),s(c.redUser)].filter(Boolean).join(' '));if(c.ingresos)out.push('Relación/ingresos: '+s(c.ingresos));out.push('Promociones: '+(c.aceptaPromos===true?'Sí':'No'));return out.join('\n');
  }
  function copiar(){
  ...
  function fallback(txt){var t=document.createElement('textarea');t.value=txt;t.style.position='fixed';t.style.opacity='0';document.body.appendChild(t);t.select();try{document.execCommand('copy');toast('✅ Clientes copiados para CRM');}catch(e){toast('No se pudo copiar.');}t.remove();}
  function descargar(){
    var a=clientes();if(!a.length)return toast('No hay clientes para descargar.');
    var cols=['Nombre','Nombre preferido','Telefono','DNI','Email','Direccion','Localidad','Red','Usuario red','Relacion/Ingresos','Acepta promociones','Creado','Actualizado'];
    var rows=[cols.map(q).join(',')];a.forEach(function(c){rows.push([c.nombre,c.apodo,c.telefono||c.tel,c.dni,c.mail||c.email,c.domicilio||c.dir,c.localidad||c.loc,c.redTipo,c.redUser,c.ingresos,c.aceptaPromos===true?'Si':'No',c.creado,c.actualizado].map(q).join(','));});
  ...
  function ponerBarra(){
    ...
    var b=document.createElement('div');b.id='ae-crm-export-bar';b.style.cssText='display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:9px 0 12px';b.innerHTML='<button id="ae-crm-copy" type="button" style="border:0;border-radius:11px;padding:10px;background:#0B2D6B;color:#fff;font:900 .68rem inherit">📋 Copiar para CRM</button><button id="ae-crm-csv" type="button" style="border:0;border-radius:11px;padding:10px;background:#16a34a;color:#fff;font:900 .68rem inherit">⬇️ Descargar CSV</button>';titulo.insertAdjacentElement('afterend',b);b.querySelector('#ae-crm-copy').onclick=copiar;b.querySelector('#ae-crm-csv').onclick=descargar;
  }
  function envolver(){
  ...
  window.AmarangoCRMExport={copiar:copiar,descargarCSV:descargar};
  envolver();
})();
