/* AmarangoElectro · ajustes UX Margarita + voz
   ...
   - Dictado estilo WhatsApp dentro de Margarita: contador limpio + envío con flecha.
   - Dictado simple en el buscador de la tienda.
*/
(function(){
  'use strict';
  const VERSION='mg-ux-voz-20260825-6';
  ...
  function estilos(){
    ...
    const s=document.createElement('style');s.id='mg-ux-voz-style';s.textContent=`
      ...
      .mg-voice-mode #margarita-input{color:transparent!important;-webkit-text-fill-color:transparent!important;caret-color:transparent!important;text-shadow:none!important;font-size:0!important;pointer-events:none!important;background:#fff!important}
      .mg-voice-mode #margarita-input::placeholder{color:transparent!important;-webkit-text-fill-color:transparent!important;opacity:0!important}
      .mg-voice-status{display:none;position:absolute;left:22px;top:50%;transform:translateY(-50%);z-index:8;height:42px;align-items:center;gap:9px;color:#334155;font:800 .88rem/1 system-ui,-apple-system,Segoe UI,sans-serif;pointer-events:none;white-space:nowrap}
      .mg-voice-mode .mg-voice-status{display:flex}
      .mg-voice-dot{width:10px;height:10px;min-width:10px;border-radius:50%;background:#ef4444;animation:aeVoicePulse 1s ease-in-out infinite}
      .mg-voice-time{min-width:42px;font-variant-numeric:tabular-nums;color:#0B2D6B;font-size:.96rem;letter-spacing:.01em}
      ...
      @media(max-width:390px){.ae-voice-search{right:44px!important}.ae-voice-wrap input{padding-right:88px!important}.mg-voice-status{left:18px;gap:7px}.mg-voice-time{min-width:40px}}
  ...
  function statusVoz(bar,on){
    ...
    if(on){
      if(!st){st=document.createElement('div');st.className='mg-voice-status';st.innerHTML='<span class="mg-voice-dot"></span><span class="mg-voice-time">0:00</span>';bar.appendChild(st);}
  ...
  function terminarVozMargarita(enviar){recEnviarAlFinal=!!enviar;try{recActivo&&recActivo.stop();}catch(e){}}
  function iniciarVozMargarita(btn,input,bar){
    ...
    r.onend=()=>{btn.classList.remove('on');statusVoz(bar,false);recActivo=null;recBtn=null;const txt=String(recTexto||input.value||'').trim();if(recEnviarAlFinal&&txt&&typeof window.margaritaEnviar==='function')setTimeout(()=>window.margaritaEnviar(),60);recEnviarAlFinal=false;};
  ...
  function microMargarita(){
    ...
    if(enviar){enviar.addEventListener('click',function(ev){if(!recActivo)return;ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation();terminarVozMargarita(true);},true);}
  }

  function reconocerBusqueda(onTexto,btn){const SR=speechCtor();if(!SR)return;const r=new SR();r.lang='es-AR';r.interimResults=false;r.maxAlternatives=1;btn.classList.add('on');r.onresult=e=>{const t=String(e.results?.[0]?.[0]?.transcript||'').trim();if(t)onTexto(t);};r.onend=()=>btn.classList.remove('on');r.onerror=()=>btn.classList.remove('on');try{r.start();}catch(e){btn.classList.remove('on');}}
